...
const duplicador= numeros=>{
    let numerosNuevos=new Array;
    let i=0;
    numeros.forEach(numero => {
        if(numero%2==0){
            numerosNuevos[i]=numero*2;
            i++;
        }
 ...
 */
 const eliminarDuplicados= numeros=>{
    ...
    let numeroEli=new Array;
    numeros.forEach((numero,indice) => {
        let i=0;
        numerosNuevos.forEach((numeroN,indiceN)=>{
            if (numero==numeroN){
                i++;
            }
        });
        if (i==1){
            numeroEli.push(numero);
        }
    });
    console.log(numeroEli);
}
 ...
 * 
 */
const nCharConsec=((busqueda,posCons,cadena)=>{
    let cont=0;
    let flag=false;
...
const generador=size=>{
    ...
    console.log(array);
    array=array.filter(numero=> numero!=0)
    console.log(array);
    ...
    let i=1;
    for (indice in array){
        valorMed+=array[indice];
        mapa.set(array[indice],valorMed/i);
        i++  
    }
    console.log(mapa);
}
