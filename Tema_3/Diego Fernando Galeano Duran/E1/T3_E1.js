...
function formatStr(textToFormat){
    let textWithoutSpaces=textToFormat.replaceAll(" ","");
    let formattedText="";
    for(i=0;i<textWithoutSpaces.length;i++){
        if(consonantes.indexOf(textWithoutSpaces.charAt(i).toUpperCase())>0){
            formattedText+=textWithoutSpaces.charAt(i).toUpperCase();
        }else{
            formattedText+=textWithoutSpaces.charAt(i)
        }
    }
...
function esPalindromo(textToAnalyze){
    let textWithoutSpaces=textToAnalyze.replaceAll(" ","");
    let isPalindrome=true;
    for(i=0;i<(textWithoutSpaces.length/2);i++){
        if(textWithoutSpaces.charAt(i).toUpperCase()!=textWithoutSpaces.charAt((textWithoutSpaces.length-i-1)).toUpperCase()){
            isPalindrome=false; 
        }
    }
    return isPalindrome;
