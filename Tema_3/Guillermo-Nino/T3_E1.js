...
const obtenerLetraDni = (num) => num < 0 || num > 99999999 ? null : num.toString()+letras[num % 23];
 ...
 */
const esPar = (cifra) => cifra % 2 === 0 ? true : false;
...
//const formatStr = cadena => cadena.replace(/ /g,"").replace(/[aeiou]/gi,);
//}
const formatStr = (cadena) => {
    var vocales = 'aeiou';
    var res = cadena.replace(/ /gi,"").split('').map(letra => (vocales.indexOf(letra) > -1) ? letra.toLowerCase() : letra.toUpperCase());
    return res.join("")
}
 ...
 */
 const esPalindromo = (frase) => {
    var frase_formateada= frase.replace(/[^A-Za-z0-9]/g,"").toLowerCase();
    return frase_formateada === frase_formateada.split('').reverse().join('');
}