 ...
 */
const duplicador = (array) => array.filter(numero => numero % 2 === 0).map(x => x*2);
 ...
 */
const media = array => array.reduce((total,value) => total + value) / array.length;
 ...
 */
const eliminarDuplicados = lista => lista.filter(num => lista.indexOf(num) === lista.lastIndexOf(num));
 ...
 * 
 */
const nCharConsec = (char,occ,string) => RegExp(`\\${char}`.repeat(occ),"g").test(string);
...
const generador = numero => {
    var array = Array.from(Array(numero)).map(x => Math.floor(Math.random()*10)); //Creamos array con numeros aleatorios del 1 al 10
    array = array.filter((item,index) => array.indexOf(item)===index).filter(num => num > 0); //Filtramos los numeros mayores que 0 y quitamos duplicados
    // Creamos el mapa
    var mapa = new Map;
    for(let i=0;i<array.length;i++) {
        mapa.set(array[i],array.slice(0,i+1).reduce(reducer = (acum,value) => acum+value)/(i+1));
    }
    return mapa
}