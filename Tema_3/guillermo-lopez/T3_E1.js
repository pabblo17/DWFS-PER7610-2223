...
const obtenerLetraDni = (numeroDni) => {
    ...
    let result = numeroDni % 23;
    return numeroDni + letras[result];
}
 ...
 const esPar = (numeroEntrada) => {
    if ((numeroEntrada % 2) != 0) return false;
    return true;
 }
 console.log(esPar(2));
 console.log(esPar(9995));
  ...
  */
 const formatStr = (cadenaEntrada) => {
    let cadenaSalida = "";
    for (let i = 0; i < cadenaEntrada.length; i ++) {
        if (cadenaEntrada[i] == " ") {
            cadenaSalida += "";
        }
        else if ("bdfghjklmnñpqrstvxyz".indexOf(cadenaEntrada[i].toLowerCase()) >= 0) {
            cadenaSalida += cadenaEntrada[i].toUpperCase();
        }
        else {
            cadenaSalida += cadenaEntrada[i];
        }
    }
    return cadenaSalida;
 }
  ...
  */
 const esPalindromo = (cadenaEntrada, formatstrCallback) => {
    cadenaEntrada = formatstrCallback(cadenaEntrada);
    let cadenaCalculada = "";
    for (let i = cadenaEntrada.length - 1; i >= 0; i --) {
        cadenaCalculada += cadenaEntrada[i];
    }
    if (cadenaCalculada == cadenaEntrada) {
        return true;
    }
    return false;
 }
 console.log(esPalindromo("Dabale arroz a la zorra el abad", formatStr));