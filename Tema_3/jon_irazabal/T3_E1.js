...
const obtenerLetraDni = (numero) => {
    if (numero > 0 & numero < 99999999)
        return new String(numero).concat(letras[numero % 23]);
}
console.log("DNI con letra: " + obtenerLetraDni(50487965));
...
console.log("Es par 50487965: "+esPar(50487965));
...
const formatStr = (texto) => {
    return texto.replaceAll(' ', '').split('').map(character => {
...
console.log("Prueba de cadena de texto: "+formatStr("Prueba de cadena de texto"));
/**
 * Funcion 3
 * 
 * Palindromos
...
const esPalindromo = (texto) =>
    texto.replaceAll(' ', '').toLowerCase().split('').join('') === texto.replaceAll(' ', '').toLowerCase().split('').reverse().join('');

console.log("es Palindromo (Dabale arroz a la zorra el abad): "+esPalindromo("Dabale arroz a la zorra el abad"));
console.log("es Palindromo (No es palindromo): " +esPalindromo("No es palindromo"));