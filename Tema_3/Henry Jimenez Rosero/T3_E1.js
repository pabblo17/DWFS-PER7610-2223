...
const obtenerLetraDni =  (numero) => {salida = (numero>0&&numero<99999999) ? numero+letras[numero % 23] : 'null' ; return salida};
console.log(obtenerLetraDni(50487965));
console.log(obtenerLetraDni(0));
 ...
 */
 const esPar =  (numero) => {salida = (numero % 2) ? 'false' : 'true' ; return salida};
...
const formatStr =  (cadena) => {var salida = cadena.replace(/\s/g, "").split('');var sincon=salida.map(function(x){(x!='a'&&x!='A'&&x!='e'&&x!='E'&&x!='i'&&x!='I'&&x!='o'&&x!='O'&&x!='u'&&x!='U'? x=x.toUpperCase(): x );return x; }); return sincon.join("");};
 ...
 */
const esPalindromo =  (cadena) => {let salidax = cadena.replace(/\s/g, "").split('');let sinconx = salidax.map(function(x){(x=x.toUpperCase());return x;});const salida2x=sinconx;const salida3x=sinconx.reverse();return(salida3x==salida2x);};
console.log('1'+esPalindromo("Dabale arroz a la zorra el abad"));
