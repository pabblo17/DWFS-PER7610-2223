...
function initialize(size) {
    for (let i = 0; i < size; i++) {
        cinema.set(i, new Array(size).fill('-'));
    }
}
...
function suggest(requestSize) {
    ...
    for (let entry of cinema) { // lo mismo que recipeMap.entries()
        sugg = []
        let fila = entry[1]
        // recorre cada butaca de cada fila
        for (var i = 0; i < fila.length; i++) {
            if (fila[i] == '-') {
                sugg.push('F' + entry[0] + 'B' + i)
                if (sugg.length >= requestSize) {
                    return sugg
                }
            }
        }
...
function order(requestSize) {
    ...
    sugg.forEach(element => {
        let reserva = element.replace('F','*').replace('B','*').split('*')
        let fila = cinema.get(parseInt(reserva[1]))
        fila[reserva[2]] = 'X'
        cinema.set(parseInt(reserva[1]), fila);
    })
    return sugg
}