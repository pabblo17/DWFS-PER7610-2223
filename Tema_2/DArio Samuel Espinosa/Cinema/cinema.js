...
function initialize (size) {
  cinema = new Array(size)
  for (var i = 0; i < size; i++) {
    cinema[i] = new Array(size)
  }
  for (var i = 0; i < size; i++) {
...
function suggest (requestSize) {
  ...
  for (var i = 0; i < size; i++) {
    let arytmp = cinema[i].filter(function (seat) {
      return seat == '-'
    })
    if (requestSize <= arytmp.length) {
      for (var j = 0; j < size; j++) {
        if (cinema[i][j] == '-') {
          if (seats < requestSize) {
            result += `F${i}B${j}`
            seats++
          } else return result
          if (seats < requestSize) result += ','
        }
      }
    }
  }
  if (seats < requestSize) return null
...
function order (requestSize) {
  ...
  let seastSugest = suggest(requestSize)

  if (seastSugest == null) {
    return 'no es posible hacer reservacion'
  }

  let arrayTemp = seastSugest.split(',')

  for (i = 0; i < arrayTemp.length; i++) {
    let Row = arrayTemp[i].substring(arrayTemp[i].indexOf('F') + 1, 2)
    let colum = arrayTemp[i].substring(arrayTemp[i].indexOf('B') + 1, 4)
    cinema[Row][colum] = 'X'
  }
