...
function initialize(size) {
	sala_cine =  new Array(size);
	for(let i=0; i< size; i++){
		 sala_cine[i] = new Array(10);
...
function show() {
  let estado_actual='';
  for(let i=0; i< sala_cine.length; i++){
	  for(let j=0; j<sala_cine[i].length; j++){
		  estado_actual+= sala_cine[i][j];
	  }
	  estado_actual+="\n";
  }
...
function suggest(requestSize) {
	...
	for(let i=0; i< sala_cine.length; i++){
		...
		if(cont == 0 ){
			...
			return sugerencia.substr(0, sugerencia.length - 1);;
		}
	}
	return null;
...
function order(requestSize) {
	let reserva = suggest(requestSize);
	if(reserva!= null){
		let arreglo= reserva.split(",");
		for( let i = 0; i< arreglo.length; i++){
			let fila = arreglo[i].charAt(1);
			let columna = arreglo[i].substring( arreglo[i].indexOf('B')+1 , arreglo[i].length);
			sala_cine[fila][columna] = 'X';
		}
		return "Se ha reservado los puestos " + reserva;
	}
	else{
